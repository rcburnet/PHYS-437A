...
for i in range(len(primary_list)):
    ...
    #rest of south:
        ...
        if not 40 >= coord[i][1] >= -10:
            ind_list2.append(i)

    elif coord[i][0] <= 45:
        if not 6 >= coord[i][1] >= -10:
            ind_list2.append(i)

    elif coord[i][0] >= 330:
        if not 35 >= coord[i][1] >= -5:
            ind_list2.append(i)

    elif coord[i][0] >= 315:
        if not 10 >= coord[i][1] >= -5.2:
            ind_list2.append(i)
...
new_primary_list_file = open('w4_288_primaries_after_second_cut.txt', 'w') #Change to 282 once second cut is fully applied
for i in range(len(primary_list_second_cut)):

#Similar to script in Assignment 2, except it extracts the COMPLEMENTARY
# list of Ryan's list. Ie. Galaxies that are grouped together within 1.5
# mpc of each other.
...
#Apply first cut: Remove members that are not within 1.5 Mpc of another member
...
# that are primaries who are not within 1.5 Mpc of another primary. These galaxies
# are to be cut.

#Note: first_cut_list is 459 elements long. This leaves us with 412 primaries
# left after the first cut. We still need to remove Andromeda (NGC0224) from the 
# list:
for i in range(len(primary_list)):
...
#Now first_cut_list is 460 elements long with Andromeda, leaving us with 411
# primaries after the first cut like in Ryan's paperu
...
new_primary_list_file = open('primaries_after_first_cut.txt', 'w')
